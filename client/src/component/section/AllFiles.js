...
const AllFiles = () => {
  const { uploadFile, fetchUserFilesInfo, deleteFile, renameFile } = useFileService();
  ...
  const handleDrop = (e) => {
    ...
    if (file) {
      uploadFile(file).then(() => {
        ...
        fetchData(); // Rafraîchit la liste des fichiers après le téléchargement
      });
    }
  };
      ...
      {files.map((file) => (
        <div className="file-card" key={file.id}>
          ...
          <div className="file-details">
            <h3 className="file-name">{file.name}.{file.extension}</h3>
            <p className="file-info">{file.createdAt} par {file.userName}</p>
          </div>
          <button className="file-more" onClick={(e) => handleOpenOptionModal(e, file.name, file.extension)}>⋮</button>
        </div>
      ))}
      {isModalOpen && (
        <div
        ...
        <DeleteModal
          ...
          onConfirm={() => {
            deleteFile(selectedFileName + selectedFileExtension)
              .then(() => fetchData()) // Rafraîchit la liste des fichiers après la suppression
